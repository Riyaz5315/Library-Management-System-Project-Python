...
def dashboard():
    if 'loggedin' in session:
        ...
        cur.execute("SELECT SUM(fine_amount) FROM lost_books")
        total_fine = cur.fetchone()[0] or 0
        cur.close()
        return render_template("dashboard.html", total_fine=total_fine)
    ...
    if request.method == 'POST' and 'first_name' in request.form and 'last_name' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        if account:
            ...
            try:
                ...
                return redirect(url_for('register'))  # Redirect to prevent form resubmission
...
@app.route('/add_lost_book', methods=['GET', 'POST'])
def add_lost_book():
    if request.method == 'POST':
        ...
        # Insert data into the lost_books table, including user_name
        cur = mysql.connection.cursor()
        cur.execute("""
            INSERT INTO lost_books (book_id, user_id, lost_date, fine_amount, user_name)
            VALUES (%s, %s, %s, %s, %s)
        """, (book_id, user_id, lost_date, fine_amount, user_name))
        mysql.connection.commit()
        ...
        flash('Lost book added successfully with fine!', 'success')
        # Redirect to the lost books list page or render with updated data
        return redirect(url_for('lost_books'))

    # Calculate the total fine when loading the form for GET requests
    cur = mysql.connection.cursor()
    cur.execute("SELECT SUM(fine_amount) FROM lost_books")
    total_fine = cur.fetchone()[0] or 0
    cur.close()

    # Render the form with the current total fine
    return render_template('add_lost_book.html', total_fine=total_fine)

# Example route to display lost books
@app.route('/lost_books')
def lost_books():
    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM lost_books")
    lost_books_list = cur.fetchall()
    cur.close()
    return render_template('lost_books.html', lost_books=lost_books_list)

@app.route('/remove_lost_book/<int:book_id>', methods=['POST'])
def remove_lost_book(book_id):
    cur = mysql.connection.cursor()
    cur.execute("DELETE FROM lost_books WHERE id = %s", (book_id,))
    mysql.connection.commit()
    cur.close()
